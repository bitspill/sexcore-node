...
/**
 * Will return the path and default sexcore-node configuration. It will search for the
 * configuration file in the "~/.sexcoin" directory, and if it doesn't exist, it will create one
...
function getDefaultConfig(options) {
  ...
  var defaultPath = path.resolve(process.env.HOME, './.sexcoin');
  var defaultConfigFile = path.resolve(defaultPath, './sexcore-node.json');
      ...
      servicesConfig: {
        bitcoind: {
          spawn: {
            ...
            exec: path.resolve(__dirname, '../../bin/sexcoind')
          }
        }
      }
    };
